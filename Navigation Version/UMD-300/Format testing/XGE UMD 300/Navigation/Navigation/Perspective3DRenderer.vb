...
Public NotInheritable Class Perspective3DRenderer
    ...
    Public Property MapPaperColor As Color = Color.FromArgb(70, 86, 91)
    ...
    Public Class OverlayInstance
        ...
        Public Property WorldZ As Single
    ...
    Public Property OverlayWorldHeightUnits As Single = 85.0F
    Public Property OverlayMinDrawPx As Single = 3.0F
    Public Property OverlayMaxDrawPx As Single = 50.0F

    Public Sub RenderOverlays(g As Graphics, overlays As List(Of OverlayInstance), tw As Integer, th As Integer, panelW As Integer, panelH As Integer, headingDeg As Single, truckX As Single, truckZ As Single, zoomUsedForTile As Single)
        If overlays Is Nothing OrElse overlays.Count = 0 Then Return
        If tw < 2 OrElse th < 2 OrElse panelW < 4 OrElse panelH < 4 OrElse zoomUsedForTile <= 0.0001F Then Return
        ...
        Dim worldHTilePx = OverlayWorldHeightUnits * zoomUsedForTile

        Dim projected As New List(Of (Screen As PointF, DrawH As Single, DrawW As Single, T As Single, Item As OverlayInstance))()

        For Each o In overlays
            If o.Icon Is Nothing Then Continue For

            Dim dxTile = (o.WorldX - truckX) * zoomUsedForTile
            Dim dyTile = (o.WorldZ - truckZ) * zoomUsedForTile

            Dim dxF = dxTile * cosA + dyTile * sinA
            Dim dyF = -dxTile * sinA + dyTile * cosA

            Dim denom = 1.0F - dyF / D
            If denom <= 0.0001F Then Continue For
            Dim t = Vf / denom
            If t <= 0.0F OrElse t > 1.0F Then Continue For

            Dim ry = t * groundH - 1.0F
            Dim screenY = horizonY + ry
            If screenY < horizonY - 4 OrElse screenY > panelH + 40 Then Continue For

            Dim srcHalfW = Math.Min(nearHalfW / t, maxHalfW)
            If srcHalfW < 0.5F Then Continue For
            Dim fx = (dxF + srcHalfW) / (2.0F * srcHalfW)
            If fx < -0.15F OrElse fx > 1.15F Then Continue For
            Dim screenX = fx * panelW - 0.5F

            Dim screenPerTilePx = panelW / (2.0F * srcHalfW)

            Dim drawH = worldHTilePx * screenPerTilePx
            If drawH < OverlayMinDrawPx Then Continue For
            drawH = Math.Min(drawH, OverlayMaxDrawPx)

            Dim aspect = If(o.Height > 0, o.Width / o.Height, 1.0F)
            Dim drawW = drawH * aspect

            projected.Add((New PointF(screenX, screenY), drawH, drawW, t, o))
        Next

        If projected.Count = 0 Then Return

        projected.Sort(Function(a, b) a.T.CompareTo(b.T))
        ...
        Try
            For Each p In projected
                g.DrawImage(p.Item.Icon, p.Screen.X - p.DrawW / 2.0F, p.Screen.Y - p.DrawH, p.DrawW, p.DrawH)
            Next
        Finally
