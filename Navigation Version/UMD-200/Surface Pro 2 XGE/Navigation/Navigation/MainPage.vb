...
Public Class MainPage
    ...
    Private _lastCmdRetryAttempt As DateTime = DateTime.MinValue
    Private Const CMD_RETRY_INTERVAL_MS As Double = 3000.0
...
#Region "Form Init"
    ...
    Public Sub TryReacquireOfflineCompositor()
        SyncLock _compositorLock
            If _offlineCompositor IsNot Nothing AndAlso _offlineCompositor.IsAvailable Then Return
            If (DateTime.Now - _lastCmdRetryAttempt).TotalMilliseconds < CMD_RETRY_INTERVAL_MS Then Return
            _lastCmdRetryAttempt = DateTime.Now

            Dim path = FindCmdPath(isDay:=_isDay)
            ...
            If candidate IsNot Nothing Then
                ...
                Debug.WriteLine("[LOOP] Offline-Compositor reacquired — Laufwerk wieder verfügbar.")
            End If
        End SyncLock
    End Sub
    ...
    Private Sub OfflineTileLoop()
        ...
        While _running AndAlso _offlineMap
            Try
                SyncLock _compositorLock
                    ...
                    If moved Then
                        ...
                        Debug.WriteLine($"[LOOP] requesting tile: x={_data.TruckX:F1} z={_data.TruckZ:F1} renderZoom={renderZoom:F4} tileZoom={tileZoom:F4} cw={cw} ch={ch} reqW={reqW} reqH={reqH}")

                        Dim usedZ = 0
                        Dim renderedTile = _offlineCompositor.Render(
                        CSng(_data.TruckX), CSng(_data.TruckZ),
                        tileZoom, reqW, reqH, usedZ)

                        Dim newTile As Bitmap = Nothing
                        If renderedTile IsNot Nothing Then
                            newTile = CloneBitmap32PArgb(renderedTile)
                        End If
            ...
            Catch ex As ObjectDisposedException
                Debug.WriteLine($"[LOOP] Offline-Reader-Stream invalid (DVD removed/Standby?): {ex.Message}")
                SyncLock _compositorLock
                    _offlineCompositor?.Dispose()
                    _offlineCompositor = Nothing
                End SyncLock
...
#Region "Zoom"
    ...
    Private Function GetPerspectiveZoomDivisor() As Single
        ...
        Dim frac = CSng(idx) / CSng(_offlineRenderZoomStops.Length - 1)   ' 0 = weit gezoomt, 1 = nah gezoomt
    ...
    Private Sub SaveLastScale(idx As Integer)
        ...
        Catch ex As Exception
            Debug.WriteLine($"[SCALE] Speichern von SCALE.VAL fehlgeschlagen: {ex.Message}")
        End Try
    End Sub
    ...
    Private Function LoadLastScaleIdx() As Integer
        ...
        Catch ex As Exception
            Debug.WriteLine($"[SCALE] Laden von SCALE.VAL fehlgeschlagen: {ex.Message}")
            Return fallback
        End Try
    End Function
    ...
    Private Function LoadLiquidMapSetting() As Boolean
        ...
        Try
            ...
            Return fallback
        Catch ex As Exception
            Debug.WriteLine($"[LIQUIDMAP] Lesen von LIQUIDMAP.STA fehlgeschlagen: {ex.Message}")
            Return fallback
        End Try
    End Function
